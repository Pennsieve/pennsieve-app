...
app.config.globalProperties.$sanitize = (html, allowedTags = ["br"]) =>
  striptags(html, allowedTags);
app.config.globalProperties.$message = ElMessage;
app.config.globalProperties.$pusher = new Pusher(
  siteConfig.pusherConfig.appId,
  {
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.name === "home" && token && savedOrgId) {
    ...
    if (Object.keys(store.getters.profile).length === 0) {
      await useGetProfileAndOrg(store).catch((err) => useHandleXhrError(err));
    }
    next();
  } else {
  ...
  return useGetToken().then(async (token) => {
    ...
    const teamAndPublishersPromise = useSendXhr(teamUrl)
      ...
      .then(() => {
        ...
        return useSendXhr(publisherUrl).then((resp) => {
...
async function checkActiveOrg(to) {
  ...
  const activeOrgIndex = orgs.findIndex((org) =>
    Boolean(org.organization.id === activeOrgId)
  );
