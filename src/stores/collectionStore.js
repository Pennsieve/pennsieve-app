...
export const useCollectionsStore = defineStore('collectionsStore', () => {
  ...
  const isPublicCollection = ref(true);
  ...
  */
  const getUserCollections = async (customOffset = offset.value, customLimit = limit.value) => {
    try{
      isPublicCollection.value = false;
      const token = await useGetToken()
      const url = `${siteConfig.api2Url}/collections/`
      const response = await useSendXhr(url, {
        method: 'GET',
      ...
      userCollections.value = response.collections.map(item => ({
        id: item.nodeId,
        name: item.name,
  ...
  */
  const getPublishedCollectionDOIs = async(collectionID,version,offset,limit)=>{
    try{
  ...
  */
  ...
  const removeCollection = async({ datasetId, collectionId }) => {
    try {
      ...
      if (response.ok) {
        ...
        datasetCollections.value = datasetCollections.value.filter(c => c.id !== collectionId)
  ...
  // Updated: Fixed runtimeConfig reference issue
    ...
    try {
      ...
      if (response && response.datasets) {
        const transformedDatasets = response.datasets.map(dataset => ({
          ...
          owner: `${dataset.ownerFirstName || ''} ${dataset.ownerLastName || ''}`.trim(),
          organizationName: dataset.organizationName || '',
          updatedAt: dataset.updatedAt || dataset.revisedAt || dataset.versionPublishedAt,
          doi: dataset.doi,
  ...
  const searchDatasets = async(query, limit = 25, offset = 0) => {
    try {
      const token = await useGetToken()
      const url = `${siteConfig.discoverUrl}/search/datasets?limit=${limit}&offset=${offset}&query=${encodeURIComponent(query)}&api_key=${token}`
      const response = await useSendXhr(url, {
        method: 'GET',
        header: {
          'Content-Type': 'application/json'
