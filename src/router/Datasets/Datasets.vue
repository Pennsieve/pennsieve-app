...
export default {
  ...
  mixins: [GetDatasetDoi, Request, Sorter],
  ...
  computed: {
    ...
    ...mapGetters(["hasFeature", "getPermission", "isOrgSynced"]),
  ...
  methods: {
    ...
    getDatasetIgnoreFiles: function (datasetId) {
      ...
      return useGetToken().then((token) => {
        ...
        return this.sendXhr(url)
          .then((resp) => {
            const ignoreFiles = (resp.ignoreFiles || []).map((file) => ({
    ...
    getUserRole: async function () {
      ...
      return useGetToken().then((token) => {
        ...
        return this.sendXhr(url, {}).then((role) => {
      ...
      useGetToken().then((token) => {
        ...
        return fetch(url, {
          ...
          .then((response) => {
            ...
            this.updateConcepts(sortedModels).then(() => {
    ...
    getRelationshipTypes: async function () {
      ...
      await useGetToken().then((token) => {
        return this.sendXhr(url, {
          ...
          .then((response) => {
            this.setRelationshipTypes(response).then(
              this.updateIsLoadingRelationshipTypes(false)
            );
          })
