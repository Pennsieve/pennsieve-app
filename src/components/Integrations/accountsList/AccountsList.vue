  ...
  <bf-stage element-loading-background="transparent">
    <div v-if="integrations.length > 0" class="integration-list">
      <integration-list-item
        v-for="integration in filteredAccounts"
        :key="integration.id"
    ...
    <bf-empty-page-state v-else class="empty">
      ...
      <div v-if="hasAdminRights" class="copy">
        <h2>There are no accounts yet</h2>
        <p>
          Accounts allow external services to be notified when certain events
          occur on Pennsieve. These accounts are available to all members within
          the organization and can be managed at the dataset level under
          settings.
        </p>
      </div>
      <div v-if="!hasAdminRights" class="copy">
        <h2>{{ orgName }} doesn't have any accounts yet.</h2>
        <p>Contact your administrator to get started working with accounts.</p>
      </div>
    </bf-empty-page-state>
