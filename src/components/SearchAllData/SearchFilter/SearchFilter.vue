...
export default {
  ...
  watch: {
    ...
    'filter.target': {
      handler: function() {
        if (this.filter && this.filter.type === 'model') {
  ...
  methods: {
    ...
    setTargetFilter: function(item, type) {
      ...
      const filter = mergeDeepRight(this.filter, {
        type,
        target: id,
        targetIntId: intId,
        targetLabel,
      ...
      setTimeout(function(){
        this.$refs.criteriaProperty.setPopOverVisible()
      }.bind(this), 100);
    ...
    setPropertyFilter: function(item) {
      ...
      // Focus on the next input
      setTimeout(function(){
        this.$refs.criteriaOperations.setPopOverVisible()
      }.bind(this), 100);
    },
