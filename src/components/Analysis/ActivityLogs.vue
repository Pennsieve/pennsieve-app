...
const props = defineProps(["dialogVisible", "selectedNode", "selectedApplication"]);
...
// Fetch Logs on Workflow Change from Vuex Store
    watch(()=>props.selectedNode, async (newval,oldval)=>{
        if(!props.selectedNode.uuid){return}
        try {
            isLoading.value = true;
            await store.dispatch("analysisModule/fetchWorkflowLogs",[props.selectedApplication,props.selectedNode]);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    <dialog-body>
        ...
        <div v-else>
            ...
            <div v-else>
                ...
                <el-table-column prop="CreationTime" label="Creation Time"  width="200" show-overflow-tooltip/>
