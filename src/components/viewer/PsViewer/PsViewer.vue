    ...
    <div class="bf-viewer">
      ...
      <div id="viewer-wrap">
        <viewer-toolbar v-show="!hideToolbars" class="left">
          ...
          >
            <button
              class="btn-icon"
              :class="[
                tool.eventName === viewerActiveTool
                  ? `selected ${tool.icon}`
                  : `${tool.icon}`,
              ]"
        ...
        <div v-if="!hasItems" id="empty">
          ...
          <p>
            Select a package that you want to view from your
            <router-link :to="{ name: 'datasets-list' }"> Datasets </router-link
            >.
          </p>
        </div>
        ...
        <viewer-toolbar v-show="!hideToolbars" class="right">
          <el-tooltip
          ...
          >
            ...
            >
              <component :is="palette.icon" :height="20" :width="20" />
            </button>
          </el-tooltip>
        </viewer-toolbar>
  ...
  computed: {
    ...
    hasItems: function () {
      ...
      return (
        ...
        (Object.keys(queryParams).length && queryParams.p)
      );
    },
  ...
  methods: {
    ...
    watchActiveViewer: function () {
      ...
      if (this.availablePalettes) {
        const paletteExists = find(
          propEq("eventName", this.viewerSidePanelView),
      ...
      if (this.availableTools) {
        const toolExists = find(
          propEq("eventName", this.viewerActiveTool),
          this.availableTools
        );
        if (!toolExists) {
      ...
      useGetToken()
        .then((token) => {
          ...
          return useSendXhr(url).then((response) => {
            ...
            const existingDatasetId = pathOr(
              "",
              ["content", "id"],
              this.dataset
            );
    ...
    fetchDataset: function (id) {
      useGetToken()
        .then(async (token) => {
          ...
          return this.sendXhr(url).then(async (response) => {
    ...
    computePaletteClass: function (eventName) {
      return eventName === this.viewerSidePanelView && this.viewerSidePanelOpen
        ? "selected"
        : "";
    },
    /**
    ...
    togglePanel: function (palette) {
      ...
      if (
        this.viewerSidePanelOpen &&
        this.viewerSidePanelView === palette.eventName
      ) {
        isOpen = false;
      }
      this.setSidePanel({
