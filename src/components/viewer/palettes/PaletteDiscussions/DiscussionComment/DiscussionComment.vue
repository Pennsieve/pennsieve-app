<template>
  <div class="discussion-comment">
    <div class="user-info mb-8">
      <avatar class="mr-8" :profile="owner" />
    ...
    <div class="comment-body">
      ...
      <ul class="dot-list" :class="{ visible: isHovering }">
        <li>
          <button @click="$emit('reply-click')">Reply</button>
        </li>
        <li v-if="userIsOwner">
          <button @click="onRemoveComment">Remove</button>
        </li>
      </ul>
    </div>
  </div>
...
import { propOr, compose, replace, last, head } from "ramda";
...
const transformMentions = (comment) => {
  const transform = comment.map((mention) => {
    ...
    const transformed =
      parts.length > 0
        ? `<span class="mention" id="${getId(parts)}">${getName(parts)}</span>`
        : "";
...
export default {
  ...
  methods: {
    ...mapActions("viewerModule", ["removeComment"]),
