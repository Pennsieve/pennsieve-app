<template>
  <div v-loading="isLoading" class="palette-discussions">
    <div class="action-buttons">
      <bf-button @click="openReadmeModal"> Open Dataset Description </bf-button>
    </div>
    ...
    <div v-if="hasDiscussions" class="discussions-wrap">
      <bf-discussion
    ...
    <viewer-side-panel-empty-state v-if="showEmptyState">
      ...
      <p>
        Add a comment to start a discussion. @mention a team member to send an
        them an invitation to comment.
      </p>
...
export default {
  ...
  mixins: [ImportHref, Request, Sorter],
  ...
  computed: {
      ...
      return await useGetToken()
        .then((token) => {
          ...
          if (
            packageId &&
            apiUrl &&
            this.viewerSidePanelView === "discussion"
          ) {
    ...
    sortedDiscussions: function () {
      return this.returnSort(
        "updatedAt",
        this.viewerDiscussions.discussions,
        "desc"
      );
    },
  ...
  methods: {
    ...
    getDiscussions: function () {
      this.sendXhr(this.getDiscussionsUrl)
        .then((response) => {
