...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('viewerModule', ['viewerMpp']),
      ...mapState('viewerModule', ['viewerSlideInfo']),
    ...
    beforeUnmount() {
      EventBus.$off('change-viewer-angle')
      EventBus.$off('change-slider-zoom')
      EventBus.$off('update-value')
      EventBus.$off('change-zoom-scale')
      EventBus.$off('zoom-fit-on-screen')
    },
