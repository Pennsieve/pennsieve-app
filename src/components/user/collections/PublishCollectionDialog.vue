  ...
  <div>
    ...
    >
      <div class="bf-dialog-body">
        <div v-if="showError" class="error-message">*enter a {{ errorMsg}} to publish</div>
        <el-form ref="formRef" :model="publishCollectionObject" label-width="130px">
          <!-- Collection Name -->
          <el-form-item label="Collection Name" required>
            <el-input
              v-model="publishCollectionObject.name"
              disabled />
          </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <bf-button class="create-collection-button"type="primary" :loading="isLoading" @click="validateForm">
            Publish Collection
          </bf-button>
        </div>
      </template>
    </el-dialog>
    <ConfirmPublishDialog v-if="confirmDialogVisible" :collection-details="collectionDetails" @close="closeConfirmDialog" @publishDatasetConfirmed="publishDataset"></ConfirmPublishDialog>
  </div>
...
const publishDataset = async()=>{
  try{
    const publishedId = await collectionsStore.publishCollection(publishCollectionObject.value);
    ElMessage.success("Successfully published your collection: "+publishedId)
    router.push({ path: `/my-workspace/collections/${publishCollectionObject.value.nodeId}` })
  }catch(x){
