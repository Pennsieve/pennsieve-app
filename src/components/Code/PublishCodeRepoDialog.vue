<template>
  ...
  >
    <template #header>
      <bf-dialog-header slot="title" title="Publish Latest Version" />
    </template>

    <dialog-body>
      Please confirm if you would like to Publish the Latest Version
    </dialog-body>
    <!-- Buttons -->
    <template #footer>
      <bf-button @click="handlePublishConfirm"> Publish Latest </bf-button>
...
export default {
  ...
  methods: {
    ...
    /**
     * POST to API to Publish Latest
     */
    async handlePublishConfirm() {
      try {
        await this.publishCodeRepo({ repo: this.repo });
        EventBus.$emit("toast", {
          detail: {
            type: "success",
