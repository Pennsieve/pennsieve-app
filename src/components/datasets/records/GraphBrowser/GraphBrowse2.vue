...
function transformApiResponse(data) {
  ...
  data.forEach((obj) => {
    if (
      obj.type &&
      (obj.type === "schemaRelationship" || obj.type === "schemaLinkedProperty")
    ) {
      ...
      if (hasFromNode && hasToNode) {
        const strokeColor =
          obj.type === "schemaRelationship" ? "#F9A23A" : "#011F5B";
        const type =
          obj.type === "schemaRelationship" ? "default" : "smoothstep";
        const sourceHandle =
          obj.type === "schemaLinkedProperty" ? "source-a" : "source-b";
        const targetHandle =
          obj.type === "schemaLinkedProperty" ? "target-a" : "target-b";
        ...
        if (curNodeEdges) {
          ...
          for (let i in curNodeEdges) {
            if (
              curNodeEdges[i].target === obj.to &&
              type === curNodeEdges[i].type
            ) {
              edgeMap.set(obj.from, [...curNodeEdges, obj]);
              curNodeEdges[i].label =
                curNodeEdges[i].label + " / " + obj.displayName;
