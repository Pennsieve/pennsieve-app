...
const modelRecordsUrl = computed(() => {
  ...
  return `${
    site.conceptsUrl
  }/datasets/${datasetId}/concepts/${conceptId}/instances?limit=${
    limit.value
  }&offset=${offset.value}&orderBy=${sortBy.value}&ascending=${
...
function displayValue(property) {
  ...
  const val =
    dataType === "String" ? useSanitize(propertyValue) : propertyValue;
  ...
  <div class="concept-search-results">
    <div class="results-table">
      ...
      >
        ...
        <el-table-column v-if="hasRadioSelection" width="36" :fixed="true">
          <template #default="scope">
            <el-radio
    ...
    >
      ...
      <p v-if="propertyCount === 0">
        Before you can create a record, you'll need to add some properties to
        this model.
      </p>
      <router-link
      ...
      >
        <bf-button v-if="getPermission('editor')" class="no-results-button">
          Add Properties
        </bf-button>
      </router-link>
    </bf-empty-page-state>
    ...
    >
      ...
      <p>
        ...
        <router-link :to="{ name: 'records' }" class=""> browse </router-link> a
        different concept.
      </p>
    </bf-empty-page-state>
