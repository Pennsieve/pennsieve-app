...
const ensureActiveAnnotationLayer = () => {
  ...
  if (viewerAnnotations.value.length > 0) {
    ...
    if (!hasSelected) {
      console.log(
        "No layer selected, selecting first layer:",
        viewerAnnotations.value[0]
      );
...
const setActiveTool = (activeTool) => {
  ...
  if (activeTool) {
    ...
    const methodName =
      "set" + activeTool.charAt(0).toUpperCase() + activeTool.slice(1);
...
// Computed properties
const viewerActiveTool = computed(
  () => store.state.viewerModule.viewerActiveTool
);
...
const cursorWidth = computed(
  () => props.cWidth + props.constants["CURSOROFFSET"]
);
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      if (
        !mouseDown.value &&
        !["annResize-left", "annResize-right", "annSelect"].includes(
          pointerMode.value
        )
      ) {
...
const getPreviousAnnotation = () => {
  let cursorOffset =
    (props.cursorLoc * props.cWidth - props.constants["CURSOROFFSET"]) *
    rsPeriod.value;
  let nextAnn = annCanvas.value?.findPreviousAnnotation(
    props.start + cursorOffset
  );
  return nextAnn.start - cursorOffset;
};
...
const onAnnLayersInitialized = () => {
  console.log(
    "Annotation layers initialized, ensuring active layer for annotate tool"
  );
...
const _onMouseUp = (e) => {
  console.log(
    "TSViewerCanvas: _onMouseUp called with pointerMode:",
    pointerMode.value,
    "viewerActiveTool:",
    viewerActiveTool.value
  );
  ...
  switch (pointerMode.value) {
    ...
    case "annotate":
      ...
      console.log(
        "Creating annotation - curLIndex:",
        curLIndex,
        "available layers:",
        viewerAnnotations.value.length
      );
      ...
      if (curLIndex === null || !selectedLayer || !selectedLayer.id) {
        console.error(
          "No valid selected annotation layer found for annotation creation",
          {
            curLIndex,
            selectedLayer,
        ...
        if (
          viewerAnnotations.value.length > 0 &&
          viewerAnnotations.value[0].id
        ) {
      ...
      const selectedChannels =
        store.getters["viewerModule/viewerSelectedChannels"];
      const allChannels =
        selectedChannels.length === viewerChannels.value.length ||
        selectedChannels.length === 0;

      const duration = (e.clientX - startDragCoord.x) * rsPeriod.value;
      const startTime =
        startDragTimeStamp.value +
        (startDragCoord.x - iArea.value.getBoundingClientRect().left) *
          rsPeriod.value;
...
const _onMouseDown = (evt) => {
  console.log(
    "TSViewerCanvas: _onMouseDown called with pointerMode:",
    pointerMode.value,
    "viewerActiveTool:",
    viewerActiveTool.value
  );
  ...
  if (
    viewerActiveTool.value === "annotate" &&
    !["annResize-left", "annResize-right", "annSelect"].includes(
      pointerMode.value
    )
  ) {
  ...
  switch (pointerMode.value) {
    ...
    case "annResize-right":
      ...
      annCanvas.value?.onMouseDown(
        evt.clientX - cCoord.left,
        evt.clientY - cCoord.top
      );
      break;
  }
};
  ...
  switch (viewerActiveTool.value) {
    case "pan":
      if (mouseDown.value) {
        const setStart =
          startDragTimeStamp.value -
          (e.clientX - startDragCoord.x) * rsPeriod.value;
        emit("setStart", setStart);
      } else {
        const newPointerMode = annCanvas.value?.onMouseMove(
          mX,
          mY,
          pointerMode.value,
          mouseDown.value
    ...
    case "annotate":
      if (mouseDown.value && pointerMode.value === "annotate") {
        ...
        mouseDown.value &&
        ["annResize-left", "annResize-right"].includes(pointerMode.value)
      ) {
        const newPointerMode = annCanvas.value?.onMouseMove(
          mX,
          mY,
        ...
        if (newPointerMode) {
          console.log(
            "TSViewerCanvas: annCanvas.onMouseMove returned:",
            newPointerMode
          );
...
const renderAll = (delay = 0, requestLeadingEdgeParam = true) => {
  if (
    !renderFnc.value ||
    delay !== renderThrottle.value ||
    requestLeadingEdge.value !== requestLeadingEdgeParam
  ) {
    renderFnc.value = throttle(_renderAll, delay, {
      leading: requestLeadingEdgeParam,
    });
...
const _renderAxis = () => {
  ...
  let tickOffset =
    ((pHeight.value / (2 * nrVisibleChannels.value) + 0.5) << 1) >> 1;
  ctx.lineWidth = 0.5;
  ...
  const gridSpacing =
    props.constants["XGRIDSPACING"] * Math.ceil(props.duration / 100000000);
  const nrGridLines = Math.ceil(props.duration / gridSpacing) + 1;
  const labelDecimator = Math.ceil(
    props.constants["NRPXPERLABEL"] / (gridSpacing / rsPeriod.value)
  );
  let curXLabelIdx = 1;
  ...
  for (let i = 0; i < nrGridLines; i++) {
    ...
    if (roundedCurLoc > 1) {
      ...
      const test =
        (realX / gridSpacing + gridSpacing / 10) % labelDecimator | 0;
  ...
  if (allChannels) {
    ...
    for (let ch = 0; ch < channelConfig.length; ch++) {
      ...
      if (curChannelView.selected && curChannelView.visible) {
        ...
        ctx.fillRect(
          lblStart,
          channelOffset - halfAnnotationHeight,
          lblEnd,
          annotationHeight
        );
      }
