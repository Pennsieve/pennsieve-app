  ...
  >
    ...
    <dialog-body>
      ...
      <template v-if="deleteDiscussions">
        <p>{{ annotationDescription }} - {{ annotationDateTime }}</p>
        <hr />
        <p>
          Deleting this annotation will also remove any associated discussions
          and cannot be undone.
        </p>
        <p>Are you sure you would like to remove this annotation?</p>
      </template>
...
export default {
  ...
  computed: {
    ...
    annotationDateTime: function () {
      const deleteAnnStart = pathOr(
        "",
        ["annotation", "start"],
        this.deleteAnnotation
      );
      return (
        this.getUTCDateString(deleteAnnStart) +
        " " +
        this.getUTCTimeString(deleteAnnStart)
      );
    },
