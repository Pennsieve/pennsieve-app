  ...
  mounted: function () {
    useGetToken()
      .then((token) => {
        ...
        return useSendXhr(url).then((resp) => {
          this._getLayerResponse(resp).then(() => {
  ...
  methods: {
    ...
    _getLayerResponse(resp) {
      ...
      if (resp.results.length === 0) {
        ...
        this.$store
          .dispatch("viewerModule/setAnnotations", annLayers)
          .then(() => {
    ...
    _computeRenderOptions: function (anns) {
      ...
      for (let annIdx = 0; annIdx < anns.length; annIdx++) {
        ...
        if (curAnn.duration !== 0) {
          const curAnnEnd =
            xOffset + (viewStart + viewDuration - this.start) / this.rsPeriod;
            ...
            if (curAnn.channelIds.hasOwnProperty(ch1)) {
              ...
              for (let ch2 in channelConfig) {
                if (channelConfig.hasOwnProperty(ch2)) {
                  ...
                  if (
                    curChannelView.id === curAnn.channelIds[ch1] &&
                    curChannelView.visible
                  ) {
                    channelOffset = curChannelView.rowBaseline | 0;
                    if (channelOffset < curAnn.minOffset) {
                      curAnn.minOffset = channelOffset;
                    }
                    if (channelOffset > curAnn.maxOffset) {
                      curAnn.maxOffset = channelOffset;
                    }
    ...
    checkAnnotationRange: function (RStart, REnd) {
      ...
      for (let i = 0; i < this.cachedAnnRange.length; i++) {
        ...
        if (RStart >= curBlock.start && REnd <= curBlock.end) {
          ...
          curBlock.start <= reqRange[0].start &&
          curBlock.end >= RStart
        ) {
      ...
      const annotationsTotal = this.viewerAnnotations.reduce(
        (acc, li) => (acc = li.annotations.length)
      );
      ...
      for (let i = firstIndex; i < this.cachedAnnRange.length; i++) {
        if (this.cachedAnnRange[i].start >= reqRange[curRequestIndex].start) {
          ...
          if (this.cachedAnnRange[i].end < REnd) {
            reqRange.push({
              start: this.cachedAnnRange[i].end,
              end: this.summary.span.end,
            });
          } else {
            break;
          }
      ...
      if (resp.length >= this.constants["LIMITANNFETCH"]) {
        ...
        for (let i = 0; i < this.cachedAnnRange.length; i++) {
          if (
            ...
            this.cachedAnnRange[i].start < maxStart
          ) {
            this.cachedAnnRange[i].end = maxStart;
            break;
          }
      ...
      if (resp.length > 0) {
        ...
        for (let i = 0; i < resp.length; i++) {
          ...
          if (curAnn.linkedPackage) {
            ...
            newAnn.linkedPackage = pathOr(
              "",
              ["content", "id"],
              linkedPackages[pkgId]
            );
          ...
          if (
            !isViewingMontage &&
            newAnn.channelIds.length === this.viewerChannels.length
          ) {
        ...
        this.viewerAnnotations.forEach((layer) => {
          ...
          const filteredAnns = annotations.filter(
            (ann) => layer.id === ann.layer_id
          );
    ...
    _indexOfEnd: function (annArray, val, min, max, firstIndex) {
      ...
      if (max < min) {
        ...
        while (
          pred >= 0 &&
          annArray[pred].start + annArray[pred].duration === predVal
        ) {
          pred--;
        }
        pred++;
      ...
      if (annArray[mid].start + annArray[mid].duration > val) {
        ...
        if (firstIndex) {
          while (
            index >= 0 &&
            annArray[index].start + annArray[index].duration === val
          ) {
            index--;
          }
          index++;
      ...
      useGetToken()
        .then((token) => {
          ...
          return useSendXhr(url, {
            ...
            return this.$store
              .dispatch("viewerModule/createLayer", layer)
              .then(() => {
                return this.$store
                  .dispatch("viewerModule/setActiveAnnotationLayer", layer)
                  .then(() => {
                    EventBus.$emit("toast", {
                      detail: {
          ...
          if (curAnn.allChannels) {
            ...
            if (curAnn.duration === 0) {
              ...
              ctx.fillRect(
                curAnnStartRounded,
                annotationHeight,
                curAnnEndRounded - curAnnStartRounded,
                this.pHeight - annotationHeight
              );
              ctx.beginPath();
            ...
            if (curAnn.duration === 0) {
              ...
              ctx.moveTo(
                curAnnStartRounded,
                curAnn.minOffset + halfAnnotationHeight
              );
              ctx.lineTo(
                curAnnStartRounded,
                curAnn.minOffset + halfAnnotationHeight + 8
              );
              ctx.stroke();
            }
          } else {
            ...
            if (curAnn.duration === 0) {
              ...
              ctx.lineTo(
                curAnnStartRounded,
                curAnn.maxOffset - halfAnnotationHeight
              );
              ctx.stroke();
            } else {
              // With duration
              ctx.fillRect(
                curAnnStartRounded - 1,
                curAnn.minOffset + halfAnnotationHeight,
                curAnnEndRounded - curAnnStartRounded + 1,
                curAnn.maxOffset - curAnn.minOffset - annotationHeight
              );
              ctx.beginPath();
              ctx.moveTo(
                curAnnStartRounded,
    ...
    _renderAnnotationLabels: function (ctx, anns, hideFocusedAnn) {
      ...
      for (let iAnn = 0; iAnn < anns.length; iAnn++) {
        ...
        for (let i = 0; i < curAnn.allOffsets.length; i++) {
          ctx.fillRect(
            curAnnStartRounded - 1,
            curAnn.allOffsets[i] - halfAnnotationHeight,
            curAnnEndRounded - curAnnStartRounded + 2,
            annotationHeight
          );
        ...
        if (
          ["annSelect", "annResize-left", "annResize-right"].includes(
            this.pointerMode
          ) &&
          this.viewerActiveTool == "annotate"
        ) {
          ctx.beginPath();
          ...
          if (curAnn.duration !== 0) {
            ctx.moveTo(
              curAnnEndRounded - 3,
              firstOffset - halfAnnotationHeight + 3
            );
            ctx.lineTo(
              curAnnEndRounded - 3,
              firstOffset + halfAnnotationHeight - 3
        ...
        if (
          curAnnEndRounded - curAnnStartRounded >
          curAnn.label.length * 8 + 10
        ) {
          ctx.fillStyle =
            this.a11yList.indexOf(curAnnLayer.hexColor) >= 0
              ? "black"
              : "white";
          if (!curAnn.linkedPackage) {
            ctx.fillText(
              curAnn.label,
              curAnnStartRounded + 10,
              firstOffset + halfAnnotationHeight - 6
            );
          } else {
            ctx.fillText(
              curAnn.label,
              curAnnStartRounded + 30,
        ...
        if (linkedPackageDTO && curAnnEndRounded - curAnnStartRounded >= 30) {
          const preview = pathOr(
            {},
            ["objects", "view", 1, "content"],
            linkedPackageDTO
          );
          ...
          if (fileType === "PNG") {
            ...
            useGetToken()
              .then((token) => {
                img.src = `${apiUrl}/packages/${packageId}/files/${id}?api_key=${token}`;
                if (!img.complete) {
                  img.addEventListener("load", () => this.render(), {
                    once: true,
                  });
                }
                ctx.drawImage(
                  img,
                  curAnnStartRounded,
                  firstOffset - halfAnnotationHeight,
                  27,
                  annotationHeight
                );
              })
        ...
        switch (this.pointerMode) {
          case "annResize-left":
            if (this.focusedAnn.oldDuration > 0) {
              ...
              this.focusedAnn.end =
                this.focusedAnn.start + this.focusedAnn.duration;
            } else {
              this.focusedAnn.start = this.focusedAnn.oldStart + timeDiff;
            }
            break;
        ...
        if (checkAnns) {
          ...
          for (let i = 0; i < this.renderAnn.length; i++) {
            if (this.renderAnn[i].cStart < mX && this.renderAnn[i].cEnd > mX) {
              ...
              if (
                mY > this.renderAnn[i].cY - halfAnnHeight &&
                mY < this.renderAnn[i].cY + halfAnnHeight
              ) {
                onAnn = i;
              }
          ...
          if (onAnn !== null) {
            ...
            if (
              oldFocus === null ||
              (this.focusedAnn &&
                (mX < this.focusedAnn.cStart || mX > this.focusedAnn.cEnd))
            ) {
            ...
            switch (this.viewerActiveTool) {
              case "annotate":
                if (mX <= this.focusedAnn.cStart + 10) {
                  ...
                  mX >= this.focusedAnn.cEnd - 10 &&
                  this.focusedAnn.duration > 0
                ) {
      ...
      if (this.focusedAnn) {
        switch (this.pointerMode) {
          ...
          case "annResize-right":
            ...
            this.$store
              .dispatch("viewerModule/setActiveAnnotation", this.focusedAnn)
              .then(() => {
    ...
    shouldCheckAnnotationHover: function (mY) {
      ...
      for (let o = 0; o < this.hoverOffsets.length; o++) {
        if (
          mY >= this.hoverOffsets[o] - halfAnnHeight &&
          mY <= this.hoverOffsets[o] + halfAnnHeight
        ) {
          checkAnns = true;
          break;
