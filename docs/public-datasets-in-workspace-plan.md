...
4. **Discover parity**: version selector — DONE; README on Overview — DONE; **references / external publications — DONE** (`ReferencesPanelWidget`). **How `StaticDashboard` actually lays out** (confirmed via live DOM inspection): in this version it does **NOT** use gridstack — it renders a single `.static-dashboard-grid` **flex row, `flex-wrap: nowrap`**, where each widget's `w` is its **flex-grow** weight (`flex: <w> 1 0px`); `x`/`y`/`h` are **ignored**. So one dashboard = one row; a second row is impossible in a single dashboard. **Solution (native):** **patched the `pennsieve-dashboard` library** (`PennsieveDashboard/src/components/Dashboard/StaticDashboard.vue`) so `.static-dashboard-grid` is `flex-wrap: wrap`, columns sort row-major (min-y, then x), and each column gets a `flex-basis` from its span — so a row whose spans exceed 12 wraps (a `w:12` widget drops to its own full-width row). Backward-compatible (layouts ≤12 cols unchanged). pennsieve-app now uses a **single** dashboard: README `(x0,y0,w8)` + Citation `(x8,y0,w4)` on row 0, References `(x0,y1,w12)` wraps to row 1. Widgets still need `defineOptions({inheritAttrs:false})` (fragment roots); README gated on `readmeLoaded`. **Dev wiring:** built `PennsieveDashboard` (`vite build`) + symlinked it into `pennsieve-app/node_modules/pennsieve-dashboard` (npm-link equivalent). **Release TODO:** publish `pennsieve-dashboard@1.1.1` + bump pennsieve-app's dep (the symlink is overwritten by `npm install`). Citations via DOI content-negotiation `GET https://doi.org/{doi}` + `Accept: text/x-bibliography; style=apa`, sanitized with `striptags`.); contributors w/ ORCID + funding still open; whole-dataset download (vs per-file/zip).
...
### Overview body (README + Citation + References)
- [x] **Plain CSS layout** (NOT the dashboard wrapper). `StaticDashboard` was tried and dropped — it lays widgets out as a single non-wrapping flex row (each widget's `w` = flex-grow weight; `x/y/h` ignored), so "two-up + full-width-below" wasn't expressible without either patching the library or stacking dashboards. Simpler to hand-roll: a flex `.overview-row` with README (`flex:2`) + Citation (`flex:1`) side by side, then a full-width References card below. The widget components (`MarkdownPanelWidget`, `CitationPanelWidget`, `ReferencesPanelWidget`) are rendered directly in `.overview-card`s with their own headers.
- The exploratory `pennsieve-dashboard` flex-wrap patch was **reverted** and the registry package restored (the standard workspace `DatasetOverview` still uses it unchanged).
- README: `readme.md` via `getFileContent`. Citation degrades gracefully: `citation.doi.org` returns **404** for unregistered DOIs (common on **dev**), shown as "citation not available" + Crosscite link instead of an error/retry state (CORS is `*`).
